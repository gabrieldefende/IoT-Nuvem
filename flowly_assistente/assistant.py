...
class FlowlyAssistant:
    ...
    def run(self) -> None:
        self._say("Assistente Flowly iniciado. Diga um comando.")
        ...
        while True:
            ...
            # hierarchy check (antes de chamar API)
            ...
            # completar parâmetros faltantes
            ...
            # resolver referências por nome -> ObjectId
            ok = self._resolve_param_ids(match.command.key, params)
            if not ok:
                self._say("Ok, vamos tentar novamente.")
                continue

            # confirmação para evitar erro de transcrição
            ...
            try:
                ...
                if self.settings.debug:
                    safe_params = {k: v for k, v in params.items() if "token" not in k.lower()}
                    print(f"[DEBUG] executando command={match.command.key} params={safe_params}")
                result = self._execute(match, params)
                if self.settings.debug:
                    elapsed = time.monotonic() - started
                    print(f"[DEBUG] command={match.command.key} concluído em {elapsed:.2f}s")
                self._respond_success(match.command.key, result)
            except APIError as e:
                self._say("Ocorreu um erro ao chamar a API.")
                print(f"[API ERROR] {e}")
    ...
    def _warmup_user_type(self) -> None:
        ...
        try:
            ...
            # se não der, seguimos sem bloquear; a checagem de permissão fica conservadora
            pass
    ...
    def _has_permission(self, required: str) -> bool:
        ...
        if self.user_type not in {"admin", "user"}:
            # sem informação, bloqueia apenas o que é admin
    ...
    def _confirmation_text(self, match: Match, params: Dict[str, str]) -> str:
        ...
        if c.key == "task_details":
            return f"mostrar detalhes da tarefa {params.get('task_id')}"
        if c.key == "add_comment":
            return f"adicionar comentário na tarefa {params.get('task_id')}"
        if c.key == "update_status":
            return f"alterar status da tarefa {params.get('task_id')} para {params.get('status')}"
        if c.key == "timer":
            return f"{params.get('acao')} o cronômetro da tarefa {params.get('task_id')}"
        if c.key == "assign_to_me":
            return f"atribuir a tarefa {params.get('task_id')} para você"
    ...
    def _fill_missing_params(self, command_key: str, missing: list[str], params: Dict[str, str]) -> bool:
        for name in missing:
            if name in {"task_id", "team_id"}:
                ...
                if not ans:
                    return False
                # normaliza via mesmo mapeamento do backend
                if "andamento" in ans:
    ...
    def _choose_from_matches(self, kind: str, matches: list[tuple[int, str, str]]) -> tuple[str, str]:
        # matches: [(score, id, label)] sorted desc
        top = matches[:5]
    ...
    def _resolve_team_ref(self, ref: str) -> tuple[str, str]:
        ...
        # se houver ambiguidade, pede escolha
    ...
    def _ask(self, text: str) -> None:
        ...
        Observação: TTS bloqueante via pyttsx3 pode travar o fluxo em alguns ambientes.
        Aqui mantemos não-bloqueante para evitar congelamentos durante confirmação/inputs.
        """
        print(f"[ASSISTENTE] {text}")
        try:
            self.tts.speak_async(text)
